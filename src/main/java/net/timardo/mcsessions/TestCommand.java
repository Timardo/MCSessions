...
public class TestCommand extends CommandBase {
	...
	public void execute(MinecraftServer servers, ICommandSender sender, String[] args) throws CommandException {
		ServerData server = new ServerData("bb", "localhost", false); //dummy server data, I don't have time to find IP we are connected to
		try {
		((EntityPlayerSP)sender).connection.handleDisconnect(new SPacketDisconnect(new TextComponentString("ppp"))); //disconnect from server
		ServerAddress serveraddress = ServerAddress.fromString(server.serverIP); //code taken from ServerPinger class
